...
class WarehouseExplore(Node):
    ...
    def simple_map_callback(self, message):
       """Callback function to handle simple map updates and identify shelf.
       ...
       # Convert map to grayscale OpenCV image
       map_array = np.array(self.simple_map_curr.data, dtype=np.uint8).reshape((map_info.height, map_info.width))
       map_img = cv2.cvtColor(map_array, cv2.COLOR_GRAY2BGR)
       _, binary_map = cv2.threshold(map_array, 250, 255, cv2.THRESH_BINARY)

       # Find contours of shelves
    ...
    def camera_image_callback(self, message):
        ...
        if bbox is not None and data:
            ...
            image = cv2.polylines(image, [np.int32(bbox)], True, (0, 255, 0), 2)

            shelf_msg = WarehouseShelf()
            shelf_msg.object_name = ["placeholder"]
            ...
            if PROGRESS_TABLE_GUI:
                try:
                    ...
                    self.get_logger().warn(f"[GUI] QR GUI update failed: {e}")
        else:
            self.qr_code_str = ""
    ...
    def shelf_objects_callback(self, message):
        ...
        self.get_logger().info("Received /shelf_objects message.")

        shelf_data_msg = WarehouseShelf()
        shelf_data_msg.object_name = message.object_name
        shelf_data_msg.object_count = message.object_count
        shelf_data_msg.qr_decoded = self.qr_code_str if self.qr_code_str else "No QR code detected."

        self.publisher_shelf_data.publish(shelf_data_msg)
        self.get_logger().info(f"Published /shelf_data: {shelf_data_msg.object_name}, QR: {shelf_data_msg.qr_decoded}")

        if PROGRESS_TABLE_GUI:
            obj_str = "\n".join(f"{name}: {count}" for name, count in zip(shelf_data_msg.object_name, shelf_data_msg.object_count))
            ...
            box_app.change_box_text(self.table_row_count + 1, self.table_col_count, shelf_data_msg.qr_decoded)
            box_app.change_box_color(self.table_row_count + 1, self.table_col_count, "yellow")
            self.table_row_count = 0
